...
private:	// ADAPT/DROP
	void generateCodes(const huffman<T>::node* p, uint32_t len) {
		if (p->left_ == nullptr) {
			huffman::code c = { p->sym_, len, 0 };
			huffman::codes_table_.push_back(c);
		}
		else {
			generateCodes(p->left_, len + 1);
			generateCodes(p->right_, len + 1);
		}
	}

	void generateCodesFromTable() {
		sort(huffman::codes_table_.begin(), huffman::codes_table_.end());

		huffman::code cur = { 0, 0, 0 };
		for (auto& x : huffman::codes_table_) {
...
public:		// KEEP
	...
	bool decode(const std::string& infile, const std::string& outfile) {
		...
		if (header != "HUFFMAN2") {
			return false; // error("header != \"HUFFMAN2\": wrong format");
		}
