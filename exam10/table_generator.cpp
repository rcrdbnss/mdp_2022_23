...
public:
	huffman(const frequency<T>& f) {
		...
		node* root = pq.top();
		pq.pop();
		create_table(root, sym_table_);
		sort(begin(sym_table_), end(sym_table_));
		uint8_t len = 0;
		uint32_t code = 0;
		for (auto& x : sym_table_) {
			code <<= x._len - len;
			len = x._len;
			x._code = code;
			++code;
...
int main() 
{
	...
	std::fstream os("table.bin", std::ios::binary | std::ios::out | std::ios::trunc);
	os << "HUFFMAN";

	for (const auto& e : huff.table()) {
		uint8_t sym = e._sym;
		uint8_t len = e._len;
		uint32_t code = e._code;
		os.write(reinterpret_cast<char*>(&sym), 1);
		os.write(reinterpret_cast<char*>(&len), 1);
		os.write(reinterpret_cast<char*>(&code), 4);
	}

	os.close();

	hufstr h;
	std::string str = "Riccardo Benassi";
	std::vector<uint8_t> v = h.compress(str);
	std::string dec = h.decompress(v);
